...
interface ShoppingCardProps {
    cardItems: { id: string; name: string; quantity: number; price: number }[];
    ...
    return (
        <Box sx={{ backgroundColor: 'background.paper', p: 3, borderRadius: 2, boxShadow: 3 }}>
            {cardItems.map((item, index) => (
                <Box key={item.id} className={"containerScale"} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, p: 2, backgroundColor: 'action.hover', borderRadius: 1 }}>
                    <Box>
